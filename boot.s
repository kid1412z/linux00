...
entry start
start:
	jmpi	go,#BOOTSEG    ! this statement will load cs with 0x07c0
go:	mov	ax,cs
	mov	ds,ax
	mov	ss,ax
	mov	sp,#0x400		! arbitrary value >>512 (stack pointer,make sure it
			            ! is greater than the end of the code boot.s and has
						! enough space)

! ok, we've written the message, now
load_system:
	mov	dx,#0x0000     !BIOS int 0x13 fuction 2 read code head.s from disk
	mov	cx,#0x0002     !DH -head number  DL-drive number CH-
	mov	ax,#SYSSEG
	mov	es,ax
	xor	bx,bx
	mov	ax,#0x200+SYSLEN
